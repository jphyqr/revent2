...
import {Message, Divider, Grid ,Button, Icon, Header, Image} from 'semantic-ui-react'
import Dropzone from "react-dropzone";
 class ContractorPane extends Component {
  ...
  render() {
    ...
    const { jobsStarted, jobsCompleted, isAContractor, rating, volumeTotal, contractorPhotos } =
      contractorProfile || {};
    const { clean, craftsmanship, professionalism, punctuality } =
      rating || {};

    const averageRating =
      (rating &&
        (clean + craftsmanship + professionalism + punctuality) / 4) ||
      0;
      ...
      <Grid>
        ...
        <Dropzone style={{width:"100%", marginLeft:"auto",  marginRight:"auto", display:"block", height:150,  borderStyle:"dotted"}} onDrop={this.onDrop} multiple={false} acceptedFiles={"image/jpeg,image/png,image/gif"}>
         ...
         {this.state.files[0] &&
                       ...
                       />}
         {contractorPhotos && contractorPhotos.map(photo=>(
            <Image
