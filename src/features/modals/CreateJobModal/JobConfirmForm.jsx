...
class JobConfirmForm extends Component {
  ...
  render() {
    ...
    const LeftLine = () => (
      <div style={{ height: "40px", width: "2px", backgroundColor: "blue" }} />
    );
    ...
    const { customFields, customMaterials } = draftValue || [];
    ...
    return (
      <div>
                ...
                timesSelected.map((day, dayIndex) =>
                  day.timeSlots.map(
                    (hour, hourIndex) =>
                      hour.selected && (
                        <Label>
                          {format(moment(day.timeStamp).toDate(), "ddd MMM Do")}{" "}
                          {hour.label}
                        </Label>
                      )
                  )
        ...
        <Button>Back</Button>{" "}
        <Button onClick={() => this.handlePostJob()}>Dispatch Job</Button>
      </div>
    );
  }
}
